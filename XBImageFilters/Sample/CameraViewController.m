...
- (void)setFilterIndex:(NSUInteger)filterIndex
{
    ...
    self.filterLabel.text = self.filterNameArray[self.filterIndex];
    
    NSDictionary *paths = self.filterPathArray[self.filterIndex];
    NSArray *fsPaths = paths[kFSPathsKey];
    NSArray *vsPaths = paths[kVSPathsKey];
    NSError *error = nil;
    if (vsPaths != nil) {
    ...
    NSString *filterName = self.filterNameArray[self.filterIndex];
    if ([filterName isEqualToString:@"Overlay"]) {
        NSString *path = [[NSBundle mainBundle] pathForResource:@"LucasCorrea" ofType:@"png"];
        XBGLTexture *texture = [[XBGLTexture alloc] initWithContentsOfFile:path options:@{GLKTextureLoaderOriginBottomLeft: @YES} error:NULL];
        [texture setSafeTextureParameters];
        XBGLProgram *program = self.cameraView.programs[0];
        ...
        XBGLProgram *program = self.cameraView.programs[1];
        [program bindSamplerNamed:@"s_mainTexture" toXBTexture:self.cameraView.mainTexture unit:1];
...
- (void)setupFilterPaths
{
    ...
    self.filterPathArray = @[
                            @{kFSPathsKey: @[defaultFSPath]}, // No Filter
                            @{kFSPathsKey: @[overlayFSPath], kVSPathsKey: @[overlayVSPath]}, // Overlay
                            @{kFSPathsKey: @[suckPath]}, // Spread
                            @{kFSPathsKey: @[pixelatePath]}, // Pixelate
                            @{kFSPathsKey: @[discretizePath]}, // Discretize
                            @{kFSPathsKey: @[luminancePath]}, // Luminance
                            @{kFSPathsKey: @[blurFSPath], kVSPathsKey: @[hBlurVSPath]}, // Horizontal Blur
                            @{kFSPathsKey: @[blurFSPath], kVSPathsKey: @[vBlurVSPath]}, // Vertical Blur
                            @{kFSPathsKey: @[blurFSPath, blurFSPath], kVSPathsKey: @[vBlurVSPath, hBlurVSPath]}, // Blur
                            @{kFSPathsKey: @[luminancePath, blurFSPath, blurFSPath], kVSPathsKey: @[defaultVSPath, vBlurVSPath, hBlurVSPath]}, // Blur B&W
                            @{kFSPathsKey: @[blurFSPath, sharpFSPath], kVSPathsKey: @[vBlurVSPath, hBlurVSPath]}, // Sharpen
                            @{kFSPathsKey: @[blurFSPath, blurFSPath, discretizePath], kVSPathsKey: @[vBlurVSPath, hBlurVSPath, defaultVSPath]}]; // Discrete Blur
    
    self.filterNameArray = @[@"No Filter", @"Overlay", @"Spread", @"Pixelate", @"Discretize", @"Luminance", @"Horizontal Blur", @"Vertical Blur", @"Blur", @"Blur B&W", @"Sharpen", @"Discrete Blur"];
}
...
- (IBAction)cameraButtonTouchUpInside:(id)sender 
{
    ...
    if ([self.filterNameArray[self.filterIndex] isEqualToString:@"Sharpen"]) {
        GLKMatrix2 rawTexCoordTransform = (GLKMatrix2){self.cameraView.cameraPosition == XBCameraPositionBack? 1: -1, 0, 0, -0.976};
        XBGLProgram *program = self.cameraView.programs[1];
        [program setValue:(void *)&rawTexCoordTransform forUniformNamed:@"u_rawTexCoordTransform"];
    }
