...
+ (XBGLProgram *)defaultProgram
{
    ...
    NSError *error = nil;
    XBGLProgram *program = [[XBGLProgram alloc] initWithVertexShaderFromFile:vertexShaderPath fragmentShaderFromFile:fragmentShaderPath error:&error];
    if (program == nil) {
...
{
    XBGLShaderUniform *uniform = self.uniforms[uniformName];
    uniform.value = value;
...
- (void)bindSamplerNamed:(NSString *)samplerName toTextureNamed:(GLuint)texture unit:(GLint)unit
{
    ...
    if (texture != 0) {
        [self.samplerBindings setObject:@(texture) forKey:samplerName];
    }
    else {
...
- (void)prepareToDraw
{
    ...
    for (NSString *name in [self.dirtyUniforms allKeys]) {
        XBGLShaderUniform *uniform = self.dirtyUniforms[name];
    ...
    for (id key in [self.samplerBindings allKeys]) {
        XBGLShaderUniform *uniform = self.uniforms[key];
        XBGLTexture *texture = self.samplerXBBindings[key];
        if (texture != nil) {
            [[XBGLEngine sharedEngine] bindTexture:texture.name unit:*(GLint *)uniform.value];
        }
        else {
            NSNumber *textureNumber = self.samplerBindings[uniform.name];
