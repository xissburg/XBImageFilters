...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_6_0
    CVReturn ret = CVOpenGLESTextureCacheCreate(kCFAllocatorDefault, NULL, XBGLEngine.sharedEngine.context, NULL, &textureCache);
#else
    CVReturn ret = CVOpenGLESTextureCacheCreate(kCFAllocatorDefault, NULL, (__bridge void *)XBGLEngine.sharedEngine.context, NULL, &textureCache);
#endif
    if (ret != kCVReturnSuccess) {
        if (completion) {
